...
describe("CodeMode call observation", () => {
  ...
  test("failing instead of running denies the call; the program catches the host's error", async () => {
    ...
    const refused = CodeMode.make({
      ...
      onCall: () => Effect.fail(toolError("lookup is not allowed")),
    })
    expect(
      await Effect.runPromise(
  ...
  test("sees a tool defect as the failure the program gets, and an interrupted extension call", async () => {
    const events: Array<string> = []
    ...
    expect(events).toEqual([
      'start {"type":"tool","name":"broken","input":{}}',
      "end failure:broken",
      'start {"type":"extension","extension":"slow","name":"forever","args":[]}',
      "end interrupted",
    ])
  })
