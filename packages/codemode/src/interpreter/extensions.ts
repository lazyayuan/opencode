...
import type { Call } from "../tool-runtime.js"
...
export const extensionGlobals = <R>(
  ...
  // A host function as a program function. Arguments cross in; a global's call is observed by the host's `onCall`
  // with the host's own error on failure; then whatever came back, or was thrown, crosses out so the program catches
  // a copy. Functions inside results are part of a value's API and are not observed.
  const wrap = (value: Function, label: string, describe?: (args: ReadonlyArray<unknown>) => Call): Native<R> =>
    fn<R>(builtins, value.name, value.length, (_, values) => {
      ...
      const settle = (run: Effect.Effect<unknown, unknown, R>) =>
        (describe === undefined ? run : ctx.tools.observe(describe(args), run)).pipe(
  ...
  return extensions.flatMap((extension) =>
    Object.entries(extension.globals).map(
      ([name, value]) =>
        [name, wrap(value, name, (args) => ({ type: "extension", extension: extension.name, name, args }))] as const,
    ),
  )
}
